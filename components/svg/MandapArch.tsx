  ...
  return (
    <div className={`relative ${className ?? ''}`} style={{ width: 300, minHeight: 340 }}>
      ...
      >
        <defs>
          ...
          <linearGradient id="pillarGradient" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%" stopColor="#006666" />
            <stop offset="25%" stopColor="#C9A46A" />
        ...
        {[140, 165, 190, 215, 240].map((y) => (
          <rect key={y} x="10" y={y} width="22" height="3" rx="1" fill="#C9A46A" opacity="0.6" />
        ))}
        {[140, 165, 190, 215, 240].map((y) => (
          <rect key={y} x="268" y={y} width="22" height="3" rx="1" fill="#C9A46A" opacity="0.6" />
        ))}
        ...
        <polygon points="150,2 158,18 150,14 142,18" fill="#7A2F4E" opacity="0.9" />
        <circle cx="150" cy="12" r="4" fill="#7A2F4E" />
        <circle cx="150" cy="12" r="2" fill="#F5EDE0" />

        {/* Top arch - hanging bells / decorations */}
        {[90, 120, 150, 180, 210].map((x, i) => (
          <g key={x}>
            <circle cx={x} cy={i === 2 ? 18 : i === 1 || i === 3 ? 32 : 55} r="3" fill="#7A2F4E" opacity="0.7" />
            <line
              x1={x}
        ...
        <rect x="6" y="116" width="30" height="8" rx="2" fill="#7A2F4E" opacity="0.9" />
        <rect x="264" y="116" width="30" height="8" rx="2" fill="#7A2F4E" opacity="0.9" />

        {/* Floor base */}
        <rect x="8" y="326" width="284" height="8" rx="2" fill="#7A2F4E" opacity="0.8" />
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse 70% 60% at 50% 45%, rgba(201,164,106,0.2) 0%, transparent 70%)',
        }}
      />
